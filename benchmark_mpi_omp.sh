#!/bin/bash
#
# MPI+OpenMP Benchmark Script for Energy Storms
# ==============================================
# Runs all test cases for the MPI+OpenMP implementation with different
# combinations of cores and threads, and outputs CSV report.
#
# Usage: ./benchmark_mpi_omp.sh [OPTIONS]
#
# Options:
#   -n, --runs N         Number of runs per test (default: 10)
#   -c, --cores LIST     Comma-separated list of MPI processes (default: 2,4)
#   -t, --threads LIST   Comma-separated list of OMP threads (default: 16,32)
#   -o, --output FILE    Output CSV file (default: benchmark_mpi_omp_TIMESTAMP.csv)
...
# Default cores and threads configurations
MPI_CORES="2 4"
OMP_THREADS="16 32"
...
usage() {
    cat << EOF
${BOLD}Energy Storms MPI+OpenMP Benchmark${NC}

${BOLD}USAGE:${NC}
    ./benchmark_mpi_omp.sh [OPTIONS]

${BOLD}OPTIONS:${NC}
    -n, --runs N         Number of runs per test case (default: ${NUM_RUNS})
    -c, --cores LIST     Comma-separated list of MPI process counts (default: 2,4)
    -t, --threads LIST   Comma-separated list of OMP thread counts (default: 16,32)
    -o, --output FILE    Output CSV file (default: benchmark_mpi_omp_TIMESTAMP.csv)
    -h, --help           Show this help message

${BOLD}EXAMPLES:${NC}
    # Run with default settings
    ./benchmark_mpi_omp.sh

    # Run with 5 runs, testing 1,2,4 cores and 8,16,32 threads
    ./benchmark_mpi_omp.sh -n 5 -c 1,2,4 -t 8,16,32

    # Specify output file
    ./benchmark_mpi_omp.sh -o my_results.csv

${BOLD}NOTES:${NC}
    - All combinations of cores and threads will be tested
    - For example: -c 2,4 -t 16,32 will test: (2,16), (2,32), (4,16), (4,32)
...
if [ -z "$OUTPUT_FILE" ]; then
    mkdir -p "$RESULTS_DIR"
    OUTPUT_FILE="${RESULTS_DIR}/benchmark_mpi_omp_${TIMESTAMP}.csv"
fi
...
run_single_test() {
    ...
    printf "  Running %s (cores=%d, threads=%d): " "$test_name" "$cores" "$threads"
    ...
    for i in $(seq 1 $NUM_RUNS); do
        ...
        local output
        output=$(mpirun -np $cores --bind-to none ./energy_storms_mpi_omp $test_args 2>&1) || true
    ...
    if [ "$successful_runs" -gt 0 ]; then
        ...
        # Write to CSV
        echo "$test_name,$cores,$threads,$mean,$stddev,$min,$max,$n" >> "$OUTPUT_FILE"
    else
        print_error "    All runs failed!"
        echo "$test_name,$cores,$threads,FAILED,,,," >> "$OUTPUT_FILE"
    fi
...
main() {
    print_header "Energy Storms MPI+OpenMP Benchmark"
    
    echo "Configuration:"
    echo "  • Implementation:  MPI+OpenMP"
    ...
    # Compile
    print_section "Compiling MPI+OpenMP Implementation"
    cd "$SCRIPT_DIR"
    ...
    if [ ! -x "./energy_storms_mpi_omp" ]; then
        print_error "Executable not found: ./energy_storms_mpi_omp"
        exit 1
    fi
    
    # Initialize CSV with header
    echo "test_name,mpi_cores,omp_threads,avg_time,stddev,min_time,max_time,num_runs" > "$OUTPUT_FILE"
    ...
    for test_name in "${TEST_NAMES[@]}"; do
        ...
        for cores in $MPI_CORES; do
            for threads in $OMP_THREADS; do
                run_single_test "$test_name" "$cores" "$threads"
            done
        done
        echo ""
    done
    ...
    echo ""
    printf "%-12s %8s %8s %12s %12s %12s %12s %8s\n" "TEST" "CORES" "THREADS" "AVG(s)" "STDDEV" "MIN" "MAX" "RUNS"
    echo "────────────────────────────────────────────────────────────────────────────────────────"
    
    # Read and display CSV (skip header)
    tail -n +2 "$OUTPUT_FILE" | while IFS=',' read -r test cores threads mean stddev min max runs; do
        if [ "$mean" != "FAILED" ]; then
            printf "%-12s %8s %8s %12s %12s %12s %12s %8s\n" "$test" "$cores" "$threads" "$mean" "$stddev" "$min" "$max" "$runs"
        else
            printf "%-12s %8s %8s %12s\n" "$test" "$cores" "$threads" "FAILED"
        fi
    done
